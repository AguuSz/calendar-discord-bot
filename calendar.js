...
function listEvents(auth) {
  ...
  return new Promise((resolve, reject) => {
    calendar.events.list({
      ...
      timeMax: addDays(timeMin, 2),
      maxResults: 5,
      singleEvents: true,
      orderBy: 'startTime',
    }, (err, res) => {
      console.log(events);
      if (err) reject('The API returned an erro:r ' + err)
      const events = res.data.items;


      if (events.length) {
        let eventString = 'Estos son los eventos del dia: \n'
        events.map((event, i) => {
          eventString += `\n${i + 1}. ${event.summary} | (Link: <${event.hangoutLink}>)`
        });
